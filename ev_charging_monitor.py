...
ORDER_STATES = ["ACT","NSW","NT","QLD","SA","TAS","VIC","WA"]
...
# --- Compute charger mix by power level ---
    fast = df[df['power_kw'] >= 50]
    ac = df[(df['power_kw'] >= 11) & (df['power_kw'] < 50)]
    slow = df[df['power_kw'] < 11]
    tot_ports = df['quantity'].sum()

    n_fast = len(fast)
    n_ac = len(ac)
    n_slow = len(slow)

    pct_fast = n_fast / len(df) * 100
    pct_ac = n_ac / len(df) * 100
    pct_slow = n_slow / len(df) * 100

    unknown = len(df[df['power_kw'].isna()])
    pct_unknown = unknown / len(df) * 100
...
#       f'<div style="color:#111;font-size:12px; font-weight:400;">AU-wide live snapshot</div>'
    ...
    #   (
    #       f"Fast (≥50 kW): <b>{thousands(n_fast)}</b> ({pct_fast:.1f}%) · "
    #       f"AC (11–50 kW): <b>{thousands(n_ac)}</b> ({pct_ac:.1f}%) · "
    #       f"Slow (<11 kW): <b>{thousands(n_slow)}</b> ({pct_slow:.1f}%) · "
    #       f"Unknown <b>{thousands(unknown)}</b> ({pct_unknown:.1f}%)"
    #   ),
        (
            f"Site type: Fast (≥50 kW) <b>{thousands(n_fast)}</b> · "
            f"AC (11–50 kW) <b>{thousands(n_ac)}</b> · "
            f"Slow (<11 kW) <b>{thousands(n_slow)}</b> · "
            f"Unknown <b>{thousands(unknown)}</b>"
        ),
        ...
        (
            ...
            f"Within 5 km (<b>{shares[5]:.1f}%</b>) · "
            f"10 km (<b>{shares[10]:.1f}%</b>) · "
            f"20 km (<b>{shares[20]:.1f}%</b>)"
        ),
        (
